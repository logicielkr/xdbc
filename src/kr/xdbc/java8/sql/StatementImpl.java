...
@Override
	public long getLargeUpdateCount() throws SQLException {
		if(this.stmts != null) {
			return this.stmts[0].getLargeUpdateCount();
		} else {
...
@Override
	public void setLargeMaxRows(long max) throws SQLException {
		if(this.stmts != null) {
			for(int i = 0; i < this.stmts.length; i++) {
				this.stmts[i].setLargeMaxRows(max);
			}
		} else {
...
@Override
	public long getLargeMaxRows() throws SQLException {
		if(this.stmts != null) {
			return this.stmts[0].getLargeMaxRows();
		} else {
...
@Override
	public long[] executeLargeBatch() throws SQLException {
		if(this.stmts != null) {
			long[] r = new long[0];
			for(int i = 0; i < this.stmts.length; i++) {
				r = this.stmts[i].executeLargeBatch();
			}
			return r;
		} else {
...
@Override
	public long executeLargeUpdate(String sql) throws SQLException {
		try {
			...
			if(this.stmts != null) {
				for(int i = 0; i < this.stmts.length; i++) {
					result = this.stmts[i].executeLargeUpdate(sql);
				}
			} else {
				result = this.stmt.executeLargeUpdate(sql);
			}
			if(logger.isLoggable(Level.FINE)) { logger.fine(sql); }
			return result;
		} catch (SQLException e) {
...
@Override
	public long executeLargeUpdate(String sql, int autoGeneratedKeys) throws SQLException {
		try {
			...
			if(this.stmts != null) {
				for(int i = 0; i < this.stmts.length; i++) {
					result = this.stmts[i].executeLargeUpdate(sql, autoGeneratedKeys);
				}
			} else {
				result = this.stmt.executeLargeUpdate(sql, autoGeneratedKeys);
			}
			if(logger.isLoggable(Level.FINE)) { logger.fine(sql); }
			return result;
		} catch (SQLException e) {
...
@Override
	public long executeLargeUpdate(String sql, int[] columnIndexes) throws SQLException {
		try {
			...
			if(this.stmts != null) {
				for(int i = 0; i < this.stmts.length; i++) {
					result = this.stmts[i].executeLargeUpdate(sql, columnIndexes);
				}
			} else {
				result = this.stmt.executeLargeUpdate(sql, columnIndexes);
			}
			if(logger.isLoggable(Level.FINE)) { logger.fine(sql); }
			return result;
		} catch (SQLException e) {
...
@Override
	public long executeLargeUpdate(String sql, String[] columnNames) throws SQLException {
		try {
			...
			if(this.stmts != null) {
				for(int i = 0; i < this.stmts.length; i++) {
					result = this.stmts[i].executeLargeUpdate(sql, columnNames);
				}
			} else {
			result = this.stmt.executeLargeUpdate(sql, columnNames);
			}
			if(logger.isLoggable(Level.FINE)) { logger.fine(sql); }
			return result;
		} catch (SQLException e) {
