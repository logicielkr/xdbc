...
@Override
	public void setObject(String parameterName, Object x, SQLType targetSqlType, int scaleOrLength) throws SQLException {
		if(this.cstmts != null) {
			for(int i = 0; i < this.cstmts.length; i++) {
				this.cstmts[i].setObject(parameterName, x, targetSqlType, scaleOrLength);
			}
		} else {
...
if(logger.isLoggable(Level.FINEST)) {
	before = System.currentTimeMillis();
			...
			if(this.cstmts != null) {
				for(int i = 0; i < this.cstmts.length; i++) {
					result = this.cstmts[i].executeLargeUpdate();
				}
			} else {
...
@Override
	public long getLargeUpdateCount() throws SQLException {
		if(this.cstmts != null) {
			return this.cstmts[0].getLargeUpdateCount();
		} else {
			return this.cstmt.getLargeUpdateCount();
		}
	}
@Override
	public void setLargeMaxRows(long max) throws SQLException {
		if(this.cstmts != null) {
			for(int i = 0; i < this.cstmts.length; i++) {
				this.cstmts[i].setLargeMaxRows(max);
			}
		} else {
			this.cstmt.setLargeMaxRows(max);
		}
	}
@Override
	public long getLargeMaxRows() throws SQLException {
		if(this.cstmts != null) {
			return this.cstmts[0].getLargeMaxRows();
		} else {
			return this.cstmt.getLargeMaxRows();
		}
	}
/**
 * Statement 가 여러 개인 경우 순차적으로 실행하고, 마지막 실행결과를 반환한다.
...
@Override
	public long[] executeLargeBatch() throws SQLException {
		if(this.cstmts != null) {
			long[] r = new long[0];
			for(int i = 0; i < this.cstmts.length; i++) {
				r = this.cstmts[i].executeLargeBatch();
			}
			return r;
		} else {
			return this.cstmt.executeLargeBatch();
		}
...
if(logger.isLoggable(Level.FINEST)) {
	before = System.currentTimeMillis();
			...
			if(this.cstmts != null) {
				for(int i = 0; i < this.cstmts.length; i++) {
					result = this.cstmts[i].executeLargeUpdate(sql);
				}
			} else {
...
if(logger.isLoggable(Level.FINEST)) {
	before = System.currentTimeMillis();
			...
			if(this.cstmts != null) {
				for(int i = 0; i < this.cstmts.length; i++) {
					result = this.cstmts[i].executeLargeUpdate(sql, autoGeneratedKeys);
				}
			} else {
...
if(logger.isLoggable(Level.FINEST)) {
	before = System.currentTimeMillis();
			...
			if(this.cstmts != null) {
				for(int i = 0; i < this.cstmts.length; i++) {
					result = this.cstmts[i].executeLargeUpdate(sql, columnIndexes);
				}
			} else {
...
if(logger.isLoggable(Level.FINEST)) {
	before = System.currentTimeMillis();
			...
			if(this.cstmts != null) {
				for(int i = 0; i < this.cstmts.length; i++) {
					result = this.cstmts[i].executeLargeUpdate(sql, columnNames);
				}
			} else {
